"""Orchestrate ce(z) retrieval for ALL schemes in schemes_input.xlsx.

Workflow per row:
1. Resolve coordinates (from lat/lon columns, or from postcode if absent).
2. Detect country from lat/lon and set isUK flag.
3. If UK: get_site_geo() → postcode, OS easting/northing, city, country.
4. If UK: get_distance_to_sea_km() and get_elevation_m().
5. If UK: Build scheme JSON.
6. If UK: Website: load JSON → OS datum → vbmap → Calculate → ce(z), vb0.
7. Write all results back to Excel.
"""
